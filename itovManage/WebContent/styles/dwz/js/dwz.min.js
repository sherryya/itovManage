...
(function($){
	...
	$.extend(String.prototype, {
		...
		isPhone:function() {
			return (new RegExp(/(^(\d{3,4}-)?\d{7,8}$)|(^([1][3548]\d{9})$)/).test(this));///(^([0-9]{3,4}[-])?\d{3,8}(-\d{1,6})?$)|(^\([0-9]{3,4}\)\d{3,8}(\(\d{1,6}\))?$)|(^\d{3,8}$)/).test(this));
		},
		isUrl:function(){
...
(function($){
	if ($.validator) {
		...
		$.validator.addMethod("phone", function(value, element) {
			return this.optional(element) ||/(^(\d{3,4}-)?\d{7,8}$)|(^[1][3548]\d{9}$)/.test(value);//^(\d{3,4}-)?\d{7,8})$|([1][3548]\d{9}); ///^[0-9 \(\)]{7,30}$/.test(value);
